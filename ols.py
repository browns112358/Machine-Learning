...
from numpy import log, pi, sqrt, square, diagonal, transpose
...
class ols:
    ...
    def estimate(self):
        ...
        self.sse = dot(transpose(self.e),self.e)/self.df_e         # SSE
    ...
    def dw(self):
        ...
        """
        de = diff(self.e,1)
        dw = dot(transpose(de),de) / dot(transpose(self.e),self.e);
    ...
    def ll(self):
        ...
        ll = -(self.nobs*1/2)*(1+log(2*pi)) - (self.nobs/2)*log(dot(transpose(self.e),self.e)/self.nobs)
    ...
    def summary(self):
        ...
        for i in range(len(self.x_varnm)):
            print '''% -5s          % -5.6f     % -5.6f     % -5.6f     % -5.6f''' % tuple([self.x_varnm[i],self.b[i],self.se[i].mean(),self.t[i].mean(),self.p[i].mean()]) 
        print '=============================================================================='
        print 'Models stats                         Residual stats'
        print '=============================================================================='
        print 'R-squared            % -5.6f         Durbin-Watson stat  % -5.6f' % tuple([self.R2, self.dw().mean()])
